# Abstraction principle tells us to hide unecessary method from the user 
...
class Item:
    ...
    def __init__(self, name, price, quantity= 2): 
        ...
        def __write_body(self):
            f''' Hello Person,
            Good Morning to You
            Thanks for the order of {self.name} at a quantity of {self.quantity}
            '''
        def __send_mail(self):
            pass
        def __connect_to_server(self,smpt_value):
            pass
        def send_email(self):
            self.__write_body()
            self.__send_mail()
